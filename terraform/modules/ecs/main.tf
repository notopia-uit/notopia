...
resource "aws_ecs_task_definition" "note" {
  ...
  container_definitions = jsonencode([{
    ...
    environment = concat(local.common_environment, [
      ...
      { name = "NOTOPIA_NOTE_DATABASE_URL", value = "postgresql://${var.db_username}:${var.db_password}@${var.rds_endpoint}:${var.rds_port}/note" },
...
resource "aws_ecs_task_definition" "document" {
  ...
  container_definitions = jsonencode([{
    ...
    environment = concat(local.common_environment, [
      ...
      { name = "NOTOPIA_DOCUMENT_DB_HOST", value = var.rds_endpoint },
      { name = "NOTOPIA_DOCUMENT_DB_PORT", value = tostring(var.rds_port) },
      { name = "NOTOPIA_DOCUMENT_DB_USER", value = var.db_username },
      { name = "NOTOPIA_DOCUMENT_DB_PASSWORD", value = var.db_password },
...
resource "aws_ecs_task_definition" "authorization" {
  ...
  container_definitions = jsonencode([{
    ...
    environment = concat(local.common_environment, [
      ...
      { name = "NOTOPIA_AUTHORIZATION_DATABASE_URL", value = "postgresql://${var.db_username}:${var.db_password}@${var.rds_endpoint}:${var.rds_port}/authorization" },
  ...
  container_definitions = jsonencode([
    {
      ...
      environment = [
        ...
        { name = "AUTHENTIK_POSTGRESQL__HOST", value = var.rds_endpoint },
        { name = "AUTHENTIK_POSTGRESQL__PORT", value = tostring(var.rds_port) },
        { name = "AUTHENTIK_POSTGRESQL__NAME", value = "authentik" },
        { name = "AUTHENTIK_POSTGRESQL__USER", value = var.db_username },
        { name = "AUTHENTIK_POSTGRESQL__PASSWORD", value = var.db_password },

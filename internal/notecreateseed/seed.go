...
func renderSQL(workspaceID uuid.UUID, folders []Folder, notes []Note, links []NoteLink) string {
	...
	fmt.Fprintf(&b, "INSERT INTO workspaces (id, slug, name) VALUES ('%s', '%s', '%s') ON CONFLICT (id) DO UPDATE SET slug = EXCLUDED.slug, name = EXCLUDED.name;\n\n", workspaceID, sqlString(workspaceSlug), sqlString(workspaceName))
	...
	if len(folders) > 0 {
		...
		b.WriteString(" ON CONFLICT (id) DO UPDATE SET name = EXCLUDED.name, icon = EXCLUDED.icon, workspace_id = EXCLUDED.workspace_id, parent_id = EXCLUDED.parent_id;\n\n")
	}
	...
	if len(notes) > 0 {
		...
		b.WriteString(" ON CONFLICT (id) DO UPDATE SET name = EXCLUDED.name, icon = EXCLUDED.icon, folder_id = EXCLUDED.folder_id, tags = EXCLUDED.tags, size = EXCLUDED.size;\n\n")
	}

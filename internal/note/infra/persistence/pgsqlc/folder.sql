-- name: GetFolder :one
SELECT *
FROM folders
WHERE id = sqlc.arg('id')
  AND deleted_at IS NULL;

-- name: SaveFolder :exec
INSERT INTO folders (id, name, icon, workspace_id, parent_id, created_at, updated_at, deleted_by, deleted_at)
VALUES (sqlc.arg('id'), sqlc.arg('name'), sqlc.arg('icon'), sqlc.arg('workspace_id'), sqlc.arg('parent_id'), sqlc.arg('created_at'), sqlc.arg('updated_at'), sqlc.arg('deleted_by'), sqlc.arg('deleted_at'))
ON CONFLICT (id) DO UPDATE SET
  name = EXCLUDED.name,
  icon = EXCLUDED.icon,
  parent_id = EXCLUDED.parent_id,
  updated_at = EXCLUDED.updated_at,
  deleted_by = EXCLUDED.deleted_by,
  deleted_at = EXCLUDED.deleted_at;

-- name: GetTrashedFoldersByWorkspaceID :many
SELECT *
FROM folders
WHERE workspace_id = sqlc.arg('workspace_id')
  AND deleted_at IS NOT NULL
ORDER BY deleted_at DESC;

-- name: PermanentlyDeleteFolderByID :exec
DELETE FROM folders WHERE id = sqlc.arg('id');

-- name: PermanentlyDeleteFoldersByIDs :exec
DELETE FROM folders WHERE id = ANY(sqlc.arg('ids')::uuid[]);

-- name: GetFoldersByWorkspaceID :many
SELECT *
FROM folders
WHERE workspace_id = sqlc.arg('workspace_id')
  AND deleted_at IS NULL
ORDER BY created_at DESC;

-- name: GetRootFolder :one
SELECT *
FROM folders
WHERE workspace_id = sqlc.arg('workspace_id')
  AND parent_id IS NULL
  AND deleted_at IS NULL;

-- name: GetFoldersByParentID :many
SELECT *
FROM folders
WHERE parent_id = sqlc.arg('parent_id')
  AND deleted_at IS NULL
ORDER BY created_at DESC;

...
const getNoteGraph = `-- name: GetNoteGraph :one
...
traverse (node_id, depth) AS (
    -- Base Case
    SELECT $2::text AS node_id, 0 AS depth
    UNION
    -- Recursive Step
...
type GetNoteGraphRow struct {
	Nodes interface{}
	Links interface{}
}

func (q *Queries) GetNoteGraph(ctx context.Context, arg *GetNoteGraphParams) (*GetNoteGraphRow, error) {
	row := q.db.QueryRow(ctx, getNoteGraph, arg.WorkspaceID, arg.StartNodeID, arg.MaxDepth)
	var i GetNoteGraphRow
	err := row.Scan(&i.Nodes, &i.Links)
	return &i, err
}

const getWorkspaceGraph = `-- name: GetWorkspaceGraph :one
WITH valid_notes AS (
    SELECT
     n.id,
     n.name,
...
type GetWorkspaceGraphRow struct {
	Nodes interface{}
	Links interface{}
}

func (q *Queries) GetWorkspaceGraph(ctx context.Context, workspaceID uuid.UUID) (*GetWorkspaceGraphRow, error) {
	row := q.db.QueryRow(ctx, getWorkspaceGraph, workspaceID)
	var i GetWorkspaceGraphRow
	err := row.Scan(&i.Nodes, &i.Links)
	return &i, err
}

...
export const appConfig = registerAs(APP_CONFIG, () =>
  appConfigSchema.parse({
    ...
    logLevel:
      (process.env.NOTOPIA_DOCUMENT_LOG_LEVEL as AppConfig['logLevel'] | undefined) ?? 'warn',
...
export const getDatabaseConfig = () =>
  databaseConfigSchema.parse({
    host: process.env.NOTOPIA_DOCUMENT_DB_HOST!,
    port: parseInt(process.env.NOTOPIA_DOCUMENT_DB_PORT!, 10),
    username: process.env.NOTOPIA_DOCUMENT_DB_USER!,
...
export const s3Config = registerAs(S3_CONFIG, () =>
  s3ConfigSchema.parse({
    ...
    bucketName: process.env.NOTOPIA_DOCUMENT_S3_BUCKET_NAME!,
  } satisfies S3Config)
);
...
export const kafkaConfig = registerAs(KAFKA_CONFIG, () =>
  kafkaConfigSchema.parse({
    ...
    brokers: process.env
      .NOTOPIA_DOCUMENT_KAFKA_BROKERS!.split(',')
      .map((v) => v.trim())
      .filter(Boolean),
...
export const authenticationConfig = registerAs(AUTHENTICATION_CONFIG, () =>
  authenticationConfigSchema.parse({
    ...
    audiences: process.env.NOTOPIA_DOCUMENT_AUTHENTICATION_AUDIENCES?.split(',')
      .map((v) => v.trim())
      .filter(Boolean),
  } satisfies AuthenticationConfig)
);
